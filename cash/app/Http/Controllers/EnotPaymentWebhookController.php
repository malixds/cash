...
use App\Http\Requests\EnotWebhookRequest;
...
class EnotPaymentWebhookController extends Controller
{
    public function webhook(
        EnotWebhookRequest $request,
        PaymentProviderInterface $paymentProvider,
    ): JsonResponse {
        $payload = $request->all();

        if (! $paymentProvider->verifyWebhook($payload, (string) $request->header('x-api-sha256-signature', ''))) {
        ...
        $jobDto = DB::transaction(function () use ($dto, $payload, $payment): ?PlayWalletCreateDTO {
            ...
            $lockedPayment->update([
                'status' => $dto->status(),
                'paid_at' => $dto->isPaid() ? now() : $lockedPayment->paid_at,
                'provider_payload' => $payload,
            ]);

            if ($dto->status() === PaymentsStatusEnum::CANCELED->value) {
                // Отменяем заказ только пока он не ушёл в фулфилмент.
            ...
            if ($dto->status() === PaymentsStatusEnum::REFUNDED->value) {
                // Возврат средств. Откат пополнения PlayWallet автоматически не делаем —
                // это ручная операция. Фиксируем статус платежа и выходим.
                return null;
            }
        ...
        return response()->json([
            'ok' => true,
            'message' => $jobDto === null ? 'Webhook processed.' : 'Order queued for fulfillment.',
        ]);
    }
}
